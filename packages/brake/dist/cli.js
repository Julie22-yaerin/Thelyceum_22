...
import { loadLicense, loadSession, saveLicense, saveSession, clearLicense, callSignup, callLogin, callFetchLicense, callMe, callListInstalls, callRegisterInstall, callUnregisterInstall, callReportUsage, getServerUrl, } from "./license.js";
...
function printErr(msg) { console.error(`brake: ${msg}`); }
async function reportUsageBestEffort(input) {
    try {
        await Promise.race([
 ...
 */
async function cmdActivate(rest) {
...
async function cmdLicense(rest) {
    ...
    try {
        ...
        if (err instanceof Error)
            printErr(`server unreachable: ${err.message}`);
        exit(1);
    }
...
async function cmdInstall(rest) {
    ...
    if (target === "claude-desktop" || target === "all")
        targets.push({ name: "claude-desktop", hostType: "claude-desktop" });
    if (target === "claude-code" || target === "all")
        targets.push({ name: "claude-code", hostType: "claude-code" });
    if (target === "chatgpt" || target === "all")
        targets.push({ name: "chatgpt", hostType: "chatgpt" });
    if (targets.length === 0) {
        ...
        try {
            ...
            for (const i of me.installs) {
                if (i.device_id !== deviceId)
                    continue;
                if (target !== "all" && i.host_type !== target)
                    continue;
                try {
...
async function main() {
    ...
    try {
        ...
        if (cmd === "connections")
            return await cmdConnections();
        if (cmd === "metrics")
            return await cmdMetrics();
        if (cmd === "engage") {
            const cfg = await loadConfig();
        ...
        if (cmd === "track") {
            ...
            const label = rest[1];
            if (!pidStr || !label) {
                printErr("usage: brake track <pid> <label>");
                exit(2);
            }
            const pid = parseInt(pidStr, 10);
            if (!Number.isFinite(pid) || pid <= 0) {
                printErr(`invalid pid: ${pidStr}`);
                exit(2);
            }
            const cfg = await loadConfig();
        ...
        if (cmd === "untrack") {
            const label = rest[0];
            if (!label) {
                printErr("usage: brake untrack <label>");
                exit(2);
            }
            const cfg = await loadConfig();
