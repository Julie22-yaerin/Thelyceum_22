...
export async function installClaudeCode() {
    ...
    const alreadyInstalled = existing.some((h) => Array.isArray(h?.hooks) && h.hooks.some((hh) => typeof hh?.command === "string" && hh.command.includes("brake scan")));
    if (!alreadyInstalled) {
        existing.push({
            matcher: "Bash",
...
export async function uninstallClaudeCode() {
    ...
    if (settings.hooks?.PreToolUse) {
        settings.hooks.PreToolUse = settings.hooks.PreToolUse.filter((h) => !(Array.isArray(h?.hooks) && h.hooks.some((hh) => typeof hh?.command === "string" && hh.command.includes("brake scan"))));
        await writeJsonSafe(settingsPath, settings);
    }
}
