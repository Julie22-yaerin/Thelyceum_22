...
export function makeStopAll(config = {}) {
    return async function stopAll() {
 ...
 */
export async function trackPid(pid, label, pidDir) {
...
export async function untrackPid(label, pidDir) {
    const dir = pidDir ?? process.env.BRAKE_PID_DIR ?? DEFAULT_PID_DIR;
    const safeLabel = label.replace(/[^a-zA-Z0-9_.-]/g, "_").slice(0, 64) || "agent";
    const path = join(dir, `${safeLabel}.pid`);
