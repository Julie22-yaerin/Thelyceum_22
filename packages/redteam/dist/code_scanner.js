...
export const CODE_RULES = [
    // ── BLOCKING RULES ────────────────────────────────────────────────────────
    {
        ...
        evaluate: (text) => {
            ...
            if (divZero)
                return { evidence: divZero[0], detail: "Division by literal zero" };
            ...
            if (infRec)
                return { evidence: infRec[0], detail: "Direct infinite recursion without base condition" };
    ...
    },
    {
        ...
        evaluate: (text) => {
            ...
            if (unsafeEval)
                return { evidence: unsafeEval[0], detail: "Dynamic code execution on untrusted input" };
            ...
            if (secret)
                return { evidence: secret[0], detail: "Hardcoded secret credential or webhook token" };
            return null;
        },
    },
    {
    ...
    },
    {
        ...
        evaluate: (text) => {
            ...
            if (emptyCatch)
                return { evidence: emptyCatch[0], detail: "Empty catch block swallowing error" };
            // Direct global mutation
            const globalMutate = text.match(/\b(?:window|global|globalThis)\.[a-zA-Z0-9_$]+\s*=/);
            if (globalMutate)
                return { evidence: globalMutate[0], detail: "Direct global namespace mutation" };
            return null;
        },
    },
    {
        ...
        evaluate: (text) => {
            ...
            if (floatingAsync)
                return { evidence: floatingAsync[0], detail: "Floating promise without await or .catch()" };
            return null;
        },
    },
    {
    ...
    },
    {
        ...
        evaluate: (text) => {
            ...
            if (openWithoutClose) {
                ...
                if (!hasCleanup)
                    return { evidence: openWithoutClose[0], detail: "Resource created without visible cleanup" };
            }
            return null;
        },
    },
];
export function scanCodeFlaws(text) {
    const flags = [];
