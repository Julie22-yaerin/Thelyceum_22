...
export async function installClaudeDesktop() {
    ...
    const desktop = await readJsonSafe(configPath, {});
...
export async function installClaudeCode() {
    ...
    const alreadyInstalled = existing.some((h) => Array.isArray(h?.hooks) && h.hooks.some((hh) => typeof hh?.command === "string" && hh.command.includes("redteam challenge")));
    if (!alreadyInstalled) {
        existing.push({
            matcher: "Write|Edit",
...
export async function uninstallClaudeCode() {
    ...
    if (settings.hooks?.PreToolUse) {
        settings.hooks.PreToolUse = settings.hooks.PreToolUse.filter((h) => !(Array.isArray(h?.hooks) && h.hooks.some((hh) => typeof hh?.command === "string" && hh.command.includes("redteam challenge"))));
        await writeJsonSafe(settingsPath, settings);
    }
}
