...
server.tool("challenge", challengeDescriptionFor(mode), {
    text: z.string()
...
server.tool("rebut", rebutDescriptionFor(mode), {
    text: z.string()
...
server.tool("compact", "Filter hesitation fillers (uh, um, ừm, à) and duplicate words from context without losing technical terms or logical structure.", {
...
server.tool("redteam_status", "Read the most recent challenge events from the audit log. Shows when the red team flagged reasoning, which flaws matched, and whether the verdict blocked.", {
...
server.resource("redteam://rules", "The list of flaw rules the red team watches. Read this to see what gets challenged.", async () => ({
    contents: [{
            uri: "redteam://rules",
...
server.resource("redteam://mode", "The current red team mode. 'always' = model challenges its own reasoning proactively. 'slash' = only on explicit /redteam.", async () => ({
    contents: [{
            uri: "redteam://mode",
            mimeType: "application/json",
...
server.prompt("redteam_logic_challenge", "Challenge a proposal, plan, or code edit for reasoning flaws, overconfidence, context drift, or ping-pong loops", { proposal: z.string().describe("The proposal or code edit to challenge") }, async ({ proposal }) => ({
    messages: [{
            role: "user",
