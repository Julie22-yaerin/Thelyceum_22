...
import { compactContext, CompactOptions, CompactResult } from "./compact.js";
...
const REASONING_RULES: ReasoningFlawRule[] = [
  {
    flaw: "overconfidence",
    severity: "warning",
...
export function challenge(text: string, opts: ChallengeOptions = {}): ChallengeResult {
  ...
  const defaultBlockingSet = new Set<FlawClass>([
  ...
  const action: ChallengeVerdict["action"] = blocked
    ? "block"
    : flags.length > 0
    ? "warn"
    : "allow";
