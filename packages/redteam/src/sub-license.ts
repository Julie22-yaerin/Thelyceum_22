...
export async function checkSubLicenseGate(): Promise<SubLicenseGateResult> {
  if (!existsSync(LICENSE_PATH)) return { active: false, allowed: true };
  ...
  try {
    ...
    const message = (json as { message?: string }).message ?? "Your license code isn't valid.";
    return {
      active: true,
      allowed: false,
