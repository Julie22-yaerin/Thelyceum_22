    ...
    for (const entry of readdirSync(dir)) {
        ...
        try {
            if (statSync(p).isDirectory())
                walk(p, out);
            else if (/\.(ts|tsx|js|jsx|json|md|html|css|py|go|rs|java|rb|sh|yml|yaml|txt)$/.test(entry))
                out.push(p);
        }
        catch {
        ...
        case "measure": {
            ...
            for (let pass = 0; pass < Math.max(1, passes); pass++) {
                for (const f of files) {
                    ...
                    const r = compress(text, ledger, { sourceId: f, budgetTokens: budget });
                    before += r.before.tokens;
                    after += r.after.tokens;
                    hardT += r.hardTokens;
                    softT += r.softTokens;
                    if (isLossless(r.applied))
                        lossless += r.saved;
                    else
                        lossy += r.saved;
                    for (const m of r.applied)
  ...
  ${ansi.dim}lossy (truncation)     ${ansi.reset}  ${totalSaved > 0 ? ((lossy * 100) / totalSaved).toFixed(1) : "0.0"}%  ${lossy.toLocaleString()} tokens`;
            ...
            if (before > 0) {
                ...
                try {
                    ...
                    await fs.writeFile(LAST_MEASURE_PATH, JSON.stringify({ before, after, savedFraction, measuredAt: Date.now() }, null, 2), "utf-8");
                }
                catch {
            ...
            console.log(`${ansi.dim}⚡ Powered by The Lyceum (thelyceum.site) — run \`lyceum share\` any time for this badge again${ansi.reset}`);
            break;
        }
        // ── compress ──────────────────────────────────────────────────────────
        case "compress": {
        ...
        case "install": {
            ...
            const results = target === "all" ? await installAll()
