...
function scanLiterals(lines) {
    const startInside = new Array(lines.length).fill(false);
    const endInside = new Array(lines.length).fill(false);
    let mode = "code";
    let tplBraces = 0;
    const inside = (m) => m === "sq" || m === "dq" || m === "tpl" || m === "block";
...
function snapRuns(keep, lines, scan) {
    let runStart = -1;
    for (let i = 0; i <= lines.length; i++) {
        ...
        if (!kept && runStart !== -1) {
            let a = runStart;
            let b = i - 1;
            while (a > 0 && scan.startInside[a])
                a--;
            while (b < lines.length - 1 && scan.endInside[b])
                b++;
            for (let j = a; j <= b; j++)
                keep.add(j);
            runStart = -1;
...
function slice(text, query, budgetTokens) {
    const terms = query
        .toLowerCase()
        .split(/[^a-z0-9_$.]+/)
        .filter((t) => t.length > 2);
...
function compactStackTraces(text) {
    const lines = text.split("\n");
...
function compactGitDiffHunks(text) {
...
function normalizeLogTimestamps(text) {
    const lines = text.split("\n");
    if (lines.length < 4)
        return text;
...
function strip(text) {
    const baseStripped = compactSvgBlobs(text)
        ...
        .replace(/(?<![A-Za-z0-9+/._-])[A-Za-z0-9+/]{200,}={0,2}(?![A-Za-z0-9+/._-])/g, (m) => `[thrift: ${m.length}-char base64 omitted]`)
...
function collapseRepeats(text) {
    ...
    while (i < lines.length) {
        ...
        out.push(lines[i]);
        if (run >= 3)
            out.push(`… [thrift: previous line repeated ${run - 1} more times]`);
        else
            for (let k = 1; k < run; k++)
                out.push(lines[i]);
        i += run;
...
function cap(text, budgetTokens) {
    const est = estimateTokens(text);
    if (est.tokens <= budgetTokens)
        return text;
...
function capRespectingHard(text, budgetTokens, cls) {
    const est = estimateTokens(text);
    if (est.tokens <= budgetTokens)
        return { text, hardProtected: false };
    ...
    const marker = `[thrift: even the hard data exceeded the ${budgetTokens.toLocaleString()}-token budget, ` +
...
function guardAgainstGrowth(original, compressed) {
...
export function compress(text, ledger, options = {}) {
    ...
    if (enable.dedupe && options.sourceId) {
        ...
        if (prior) {
            ...
            const withinWindow = ageCalls <= maxDedupeAgeCalls && ageTokens <= maxDedupeAgeTokens;
            if (withinWindow) {
                const pointer = `[thrift: unchanged since you read it earlier this session ` +
    ...
    const mechNote = `${applied.join(" + ")} → saved ~${saved.toLocaleString()} tokens (${Math.round((saved / Math.max(before.tokens, 1)) * 100)}%). Estimated, ±15%.`;
    const hardNote = hardProtected
