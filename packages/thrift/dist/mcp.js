...
import { compress, SeenLedger, DEFAULT_MAX_DEDUPE_AGE_CALLS, DEFAULT_MAX_DEDUPE_AGE_TOKENS, } from "./compress.js";
...
server.tool("run_lean", "Run a shell command with machine noise removed. Also checks for runaway tool loops.", {
...
server.tool("check_loop", "Runaway loop interceptor tool. Checks if an action or intent has repeated more than 2 times. Trips and intercepts execution if loop count > 2.", {
...
server.tool("search_capabilities", "Search registered tools and skills dynamically. Allows progressive capability disclosure without context window bloat.", {
...
server.tool("invoke_tool", "Execute a catalog tool by ID with automatic Savier output compression.", {
...
server.tool("get_skill_content", "Fetch the body playbook instructions for a skill by skill ID.", {
...
server.tool("compress_text", "Compress a block of text you already have — returns compressed text plus what was removed.", {
...
server.tool("thrift_report", "What thrift has actually saved, read back from the ledger.", { limit: z.number().int().min(1).max(50_000).optional() }, async ({ limit }) => {
    const s = await summarise(limit ?? 5000);
    return { content: [{ type: "text", text: JSON.stringify(s, null, 2) }] };
});
server.resource("thrift://mechanisms", "thrift://mechanisms", async () => ({
    contents: [{
...
server.prompt("thrift_compress_context", "Compress large context blocks, log outputs, or code patches to minimize token burn", { text: z.string().describe("The text or context to compress") }, async ({ text }) => ({
    messages: [{
            role: "user",
