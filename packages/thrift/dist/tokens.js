...
function structuralDensity(text) {
    if (text.length === 0)
        return 0;
    let structural = 0;
...
export function estimateTokens(text) {
    if (!text)
        return { tokens: 0, method: "heuristic", note: "empty" };
 ...
 */
export async function countExact(text, model = "claude-sonnet-4-5") {
    const key = process.env.ANTHROPIC_API_KEY;
    if (!key) {
        throw new Error("ANTHROPIC_API_KEY is not set. Exact counting calls Anthropic's count_tokens endpoint; " +
