...
function serverUrl() {
    ...
    try {
        if (existsSync(BRAKE_CONFIG_PATH)) {
            const cfg = JSON.parse(readFileSync(BRAKE_CONFIG_PATH, "utf-8"));
            if (cfg.server_url)
 ...
 * cannot hang the CLI.
 */
export async function reportUsageBestEffort(input) {
    try {
        ...
        // best effort — the ledger already captured what happened locally
    }
}
