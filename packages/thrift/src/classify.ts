...
const CODE_SIGNAL_RE =
  /\b(function|const|let|var|import|export|class|interface|type|enum|struct|impl|fn|def|async|await|new|throw|try|catch|finally)\b|=>|^\s*return\b|[{}\[\]();]$|\b[\w$]+\s*=\s*(?!=)/;
/** Currency values and limit/budget words next to digits. */
const FINANCIAL_RE =
  /(\$|€|£|¥)\s?\d[\d,.]*|\b(USD|VND|EUR|GBP|JPY|KRW|CNY|SGD|AUD|CAD)\b\s?\d[\d,.]*|\d[\d,.]*\s?\b(USD|VND|EUR|GBP|JPY|KRW|CNY|SGD|AUD|CAD)\b|\b(limit|budget|threshold|balance|amount|max|min|cap)\b[^\n]{0,14}\d|\d[^\n]{0,14}\b(limit|budget|threshold|balance|amount|max|min|cap)\b/i;
 ...
 */
export function isHardLine(line: string): boolean {
  const t = line.trim();
  if (!t) return false;
  return (
    JSON_BRACKET_RE.test(t) ||
    JSON_KEY_RE.test(t) ||
    YAML_KEY_RE.test(t) ||
    ENV_RE.test(t) ||
...
export function classify(text: string): Classification {
  ...
  const structuredRuns: StructuredRun[] = [];

  let fence: string | null = null;
  let braceDepth = 0;
  let runStart = -1;
  let runKind: "fence" | "brace" | null = null;
