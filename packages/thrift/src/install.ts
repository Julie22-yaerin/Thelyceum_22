...
function mcpCommand(): { command: string; args: string[] } {
  ...
  if (existsSync(localEntry)) return { command: process.execPath, args: [localEntry] };
...
export async function installClaudeDesktop(): Promise<InstallResult> {
  const path = claudeDesktopConfigPath();
  if (!path) {
    return { host: "claude-desktop", ok: false, note: `Unsupported platform: ${platform()}` };
  }
  const existed = existsSync(path);
  const config = await readJsonSafe<McpConfig>(path, {});
...
export async function installClaudeCode(): Promise<InstallResult> {
  ...
  const existed = existsSync(path);
  const settings = await readJsonSafe<McpConfig>(path, {});
...
export async function installChatGPT(): Promise<InstallResult> {
...
async function removeMcpEntry(path: string, host: HostType): Promise<InstallResult> {
  if (!existsSync(path)) return { host, ok: true, path, note: "Nothing installed." };
  const config = await readJsonSafe<McpConfig>(path, {});
  if (!config.mcpServers?.thrift) return { host, ok: true, path, note: "Nothing installed." };
  delete config.mcpServers.thrift;
  await writeJsonSafe(path, config);
  return { host, ok: true, path, note: "Removed. Restart the host." };
}

export async function uninstallClaudeDesktop(): Promise<InstallResult> {
  const path = claudeDesktopConfigPath();
  if (!path) return { host: "claude-desktop", ok: false, note: `Unsupported platform: ${platform()}` };
  return removeMcpEntry(path, "claude-desktop");
}

export async function uninstallClaudeCode(): Promise<InstallResult> {
  return removeMcpEntry(join(homedir(), ".claude", "settings.json"), "claude-code");
}

export async function uninstallChatGPT(): Promise<InstallResult> {
  const dest = join(THRIFT_SKILL_DIR, "thrift", "SKILL.md");
