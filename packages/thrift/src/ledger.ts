...
export function isLossless(applied: Mechanism[]): boolean {
  return !applied.includes("cap") && !applied.includes("slice");
}

function ledgerPath(override?: string): string {
  return override ? resolve(override) : DEFAULT_LEDGER_PATH;
}

export async function record(result: CompressResult, sourceId?: string, pathOverride?: string): Promise<void> {
 ...
 * reason.
 */
export async function summarise(limit = 5000, pathOverride?: string): Promise<Summary> {
  const path = ledgerPath(pathOverride);
  const empty: Summary = {
  ...
  try {
    ...
    while (position > 0 && lines.length <= limit) {
      ...
      for (let i = parts.length - 1; i >= 0; i--) if (parts[i]) lines.push(parts[i]);
    }
  ...
  for (const e of entries) {
    ...
    if (e.lossless) lossless += e.saved;
    else lossy += e.saved;
    for (const m of e.applied) byMechanism[m] = (byMechanism[m] ?? 0) + 1;
  }
