...
class LoopTracker {
  private history: Map<string, { count: number; firstSeen: number; lastSeen: number }> = new Map();

  /** Normalize intent/action string into a hash signature */
  private getSignature(actionKey: string): string {
    return actionKey.trim().toLowerCase().replace(/\s+/g, " ");
  }

  /** Record an action and check if it exceeds the max 2 repetitions threshold */
  public trackAndCheck(actionKey: string, payloadLength = 500): LoopCheckResult {
