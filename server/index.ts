...
// ── Route modules ───────────────────────────────────────────────────────────
// index.ts had grown to 1600 lines and 72 routes; the knowledge graph showed
// createApiApp() as the second-largest hub in the codebase. Routes are grouped
// by the job an operator is doing, not by which module the handler calls.
...
export function createApiApp(): express.Express {
  ...
  registerBrainRoutes(app, authenticateLicenseKey);

  registerAutonomyRoutes(app, authenticateLicenseKey);

  registerPlansRoutes(app, authenticateLicenseKey);

  registerIntegrationsRoutes(app, authenticateLicenseKey);

  registerWorkforceRoutes(app, authenticateLicenseKey);

  registerGovernanceRoutes(app, authenticateLicenseKey);
