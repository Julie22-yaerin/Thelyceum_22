...
export const VALID_MODES: BrakeMode[] = ["always", "slash"];
...
export function isValidMode(s: string): s is BrakeMode {
  return (VALID_MODES as string[]).includes(s);
}

export async function getMode(): Promise<BrakeMode> {
  const cfg = await loadConfigFile();
  const m = cfg.mode;
  return isValidMode(typeof m === "string" ? m : "") ? (m as BrakeMode) : "always";
}

export async function setMode(mode: BrakeMode): Promise<void> {
...
export function brakeDescriptionFor(mode: BrakeMode): string {
  if (mode === "always") {
    return [
...
export function dangerScanDescriptionFor(mode: BrakeMode): string {
  if (mode === "always") {
    return [
